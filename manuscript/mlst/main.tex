...
The variables available at inference determine which scientific estimator is appropriate. A deterministic calculation is suitable when they determine the target. Partial observations require either additional measurements or a distributional assumption. Composed quantum channels make this distinction explicit because per-step descriptions and measurements of the deployed process contain different information.
...
We examine this constraint with a non-Markovian collision process. The inputs are reset-bath marginal Choi matrices, while the target comes from joint system-bath propagation and depends on the omitted retention variable $\eta$. Counterfactual replay fixes every input and varies $\eta$. The resulting target spread gives estimator-independent error bounds. An exchange-coupled audit with noncommuting interaction and bath terms tests whether the ambiguity extends beyond the original dephasing construction.

The analysis motivates a constrained convex combination of the marginal prediction and a low-shot DFE estimate from the same query. The fitted weight shifts from the prior toward the measurement as the shot budget grows. Calibration labels, pilots, and tests use independent outcomes, and the cost analysis counts every shot. A fresh OOD dataset separates the random streams for collision parameters and $\eta$. We also test symmetric readout errors and two-stage shot allocation.
...
\item A second audit uses 1024 exchange-coupled sequences with noncommuting interaction and bath terms. It recovers the same information failure with a mean ambiguity diameter of $0.100$.
...
\item The benchmark includes exact composition, low-capacity regressors, Monte Carlo, stratified DFE, independent random streams, symmetric readout errors, and full amortised costs. These controls define when to compose, measure, learn, or combine estimators.
\end{enumerate}
...
Two physical instances may produce the same estimator input while differing in a hidden variable. If their fidelities differ, every estimator using only that input has an error floor before training.
...
\end{equation}
For targets near the two ends of the range, the triangle inequality gives $\Delta_{\phi}(x)\leq |F(z)-h(x)|+|h(x)-F(z')|$, so one error is at least half the range. This bound is pointwise and worst-case. Under a conditional distribution $p(F\mid x)$, the median minimises MAE and gives the Bayes risk
\begin{equation}
...
Each regime contains training, calibration, ID test, length-OOD, and distribution-shift splits. The first three use lengths in $\{2,4,8,16\}$. The length-OOD split uses $\{24,32,48\}$. R1 and R3 hold out a noise family. R2 holds out the high-memory interval $\eta\in[0.85,0.99]$.

The main metric is pooled MAE on $\Fe$ across five independent seed-controlled splits. We also report post-hoc affine recalibration as described in \ref{app:recalibration}, and count quantum shots for measurement-based estimators. The supplementary material reports CRPS, expected calibration error (ECE), and quantile coverage. Tables give five-seed means unless specified otherwise, with standard deviations for neural results.
...
Each channel $\Lset_i$ enters through its Choi matrix $\Choi(\Lset_i) \in \mathbb{C}^{d^2 \times d^2}$. We Hermitian-symmetrise it and separate the real and imaginary parts, giving $2d^4$ real values per channel. A two-layer MLP with width $D=256$ and GELU activations maps this feature to a token $z_i \in \mathbb{R}^{D}$. The PTM encoder in Supplementary Sec.~S4 uses 16 values per single-qubit channel and differs in pooled collision MAE by less than $0.003$.

\subsection{Causal transformer backbone}

A depth-$6$ causal transformer processes $z_1, \ldots, z_n$ with eight attention heads, width 256, a fourfold feed-forward expansion, and rotary positions. Causal masking improves length-OOD stability. Bidirectional attention performs slightly better on ID and similarly after OOD calibration, as reported in \cref{sec:experiments}.
...
The fitted residual is small relative to the OOD bias, as shown in \ref{app:recalibration}. The same correction is applied to every predicted quantile.

Across five seeds, the slope standard deviation is near $0.09$ and the post-calibration MAE standard deviation is $1.3\times10^{-3}$. The correlated coefficients appear in \ref{app:recalibration}.
...
\begin{tabular}{lccc}
\toprule
Method & ID & Length OOD & Noise-family OOD \\
\midrule
...
\begin{table}[h]
\centering
\caption{\textbf{Counterfactual information audits.} Both families use 15 retention values in $[0.85,0.99]$. The model receives the same marginal input at every value within a group.}
\label{tab:ambiguity}
\small
\begin{tabular}{lcc}
\toprule
...
Maximum input change across $\eta$ & $0.0$ & $0.0$ \\
Mean diameter $\mathbb{E}[\Delta_\phi]$ & $0.2292$ & $0.1000$ \\
Mean minimax floor $\mathbb{E}[\Delta_\phi/2]$ & $0.1146$ & $0.0500$ \\
Uniform-$\eta$ Bayes MAE floor $R^\star_\phi$ & $0.0600$ & $0.0247$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph*{Independent noncommuting audit.}
The exchange model uses an excitation-exchange interaction, a dispersive $ZZ$ term, and local bath precession. The exchange and precession terms do not commute. The bath begins in $0.8|0\rangle\langle0|+0.2|1\rangle\langle1|$. Across 1024 independently drawn parameter sequences, the mean ambiguity diameter is $0.1000$. It increases from $0.0617$ at length 8 to $0.1372$ at length 24. Exact composition agrees with joint propagation at $\eta=0$ to within $5.6\times10^{-16}$, while every reset-bath marginal remains unchanged across the retention grid. The omitted-memory limitation therefore persists outside the commuting dephasing model. The supplementary material gives the Hamiltonian and length-resolved results.
...
\paragraph*{Nonideal measurements and allocation.}
We apply an independent symmetric sign flip to each sampled Pauli outcome with probability up to 5\%. A calibrated linear inversion corrects the known attenuation before fusion. At 5\% readout error, the 32-shot and 64-shot hybrids give MAE $0.0712$ and $0.0623$. Their clean controls are $0.0650$ and $0.0533$. Same-budget DFE with the same correction gives $0.1128$ and $0.0807$. Linear inversion increases variance, so it does not improve finite-shot DFE in every setting. Fusion limits this variance by reducing the fitted measurement weight.
...
Among these approximate methods, \fidno{}+cal improves over the product approximation by factors of $1.9$, $2.3$, and $1.8$ on the ID, length-OOD, and noise-family-OOD splits. Order-aware neural models also outperform the two closed-form approximations. They do not improve on exact composition.

Classical Monte Carlo gives lower error than the learned models when a sufficient marginal-simulation budget is available. MC with $K=100$ reaches MAE $0.021$ on the ID split, and $K=1000$ reduces it to $0.007$. These rows map the cost-accuracy tradeoff among approximate methods. They do not replace the deterministic result when full Markovian Choi matrices are available.
...
Method & ID & Length OOD & Noise-family OOD \\
\midrule
Exact composition  & $1.4\!\times\!10^{-8}$ & $8.3\!\times\!10^{-9}$ & $1.1\!\times\!10^{-8}$ \\
\midrule
...
The $d=4$ results repeat the negative control at a larger dimension: complete Markovian inputs favour direct composition, while Monte Carlo can outperform learned approximations when its sampling budget is affordable.

\paragraph*{GNN under family shift.}
The \textsc{generic\_gnn} model gives the lowest noise-family-OOD MAE in this regime. Its MAE is $0.039$, compared with $0.051$ for \fidno{}. This single split suggests that a path-graph inductive bias can help under some family shifts. Broader architectural comparisons require additional shifts.
...
Split & MAE & ECE & CRPS \\
\midrule
ID              & $0.025$ & $\bm{0.004}$ & $0.018$ \\
Length OOD      & $0.138$ & $0.394$ & $0.119$ \\
High-memory OOD & $0.383$ & $0.500$ & $0.359$ \\
\bottomrule
\end{tabular}
\end{table}
%%%% END flattened: sections/05_experiments
...
We use $\eps^2L^2$ only as a screening diagnostic: large values call for correlated alternatives and direct tests of marginal factorisation.
...
\midrule
Complete Markovian channel descriptions
& full per-step Choi matrices available
& exact superoperator composition \\
Weak correlations / low infidelity
& $\eps^{2}L^{2} \lesssim 10^{-3}$
& Product approximation \\
Deployed channel measurable
& query-time quantum shots available
...
& high-fidelity ranking validated on deployment data
& raw surrogate for candidate ranking only \\
\bottomrule
\end{tabular}
\end{table}

For correlated or non-Markovian channels, the choice balances simulation, per-query measurement, and one-time characterisation costs. DFE is the default when its shot count is affordable. A marginal prior suits measurement-free deployment after the shift is characterised. The conditioned hybrid occupies the middle ground and becomes cost-effective once calibration is amortised.

The uncalibrated BiDir predictions preserve broad ordering on the independent-memory split, with Spearman $\rho=0.710\pm0.002$ and Kendall $\tau=0.534\pm0.002$ across five checkpoints. The predicted highest-fidelity 10\% set overlaps the true set by $0.858\pm0.004$. The lowest-fidelity 10\% overlap is only $0.144\pm0.012$, close to the random baseline of $0.10$. The raw model can therefore support high-fidelity candidate selection in this benchmark. The same evidence does not support low-fidelity failure triage.

\subsection{QEC compatibility}

Within QEC workflows, \fidno{} provides a proxy for repeated composed-channel fidelity queries. Decoder performance, thresholds, and logical error rates require separate evaluation.
...
Its value depends on whether repeated simulation or measurement costs exceed the offline labelled-set cost.
...
The headline configuration uses $N_{\mathrm{cal}}=64$, with supplementary results down to eight. Labels may come from simulation or offline characterisation, and their cost is amortised over later queries. \ref{app:recalibration} discusses unlabelled distribution matching.

\item \emph{The input omits collision memory strength.}
For fixed collision parameters, the reset-bath marginals are independent of $\eta$. Predictions estimate a correction averaged under the deployment distribution. The realised bath state and retention value remain unobserved. The exchange-coupled audit shows that this issue is not confined to a commuting dephasing Hamiltonian, but both audits use a qubit bath.
...
\item \emph{The readout model is controlled but limited.}
...
All datasets are generated synthetically. The manuscript-specific data generators, evaluation scripts, random seeds, and machine-readable result tables are publicly available at \url{https://github.com/shchng9963-droid/fidelityno} under the release tag mlst-submission-v3. Training arrays and model checkpoints can be regenerated using the documented commands. An archival DOI will be added when the versioned release is deposited.

\section*{Code availability}

The source code associated with this manuscript is publicly available at \url{https://github.com/shchng9963-droid/fidelityno} under the release tag mlst-submission-v3. The tag identifies the exact Git commit, and the software environment is documented in the repository.
...
\Cref{tab:recal_seeds} reports an auxiliary audit with separate affine coefficients for each seed. The fit uses a labelled 10\% subset of the high-memory collision data. This calibration set and split differ from the fixed $N_{\mathrm{cal}}=64$ audit in \cref{tab:ambiguity}. Its mean MAE of $0.094$ forms a separate result from the headline $0.090$. Across five seeds, the calibrated MAE has standard deviation $1.3\times10^{-3}$ and the slope has standard deviation about $0.09$. Slope and intercept are correlated and are reported together.
...
We apply the same affine coefficients to all nine predicted quantiles. This preserves their ordering. On OOD data, empirical coverage can still differ from the nominal level. Medium and high quantiles with $q\geq0.5$ lie within $\pm0.04$ of nominal coverage. Low quantiles with $q\leq0.3$ sometimes over-cover on the high-memory split because predictions are clipped at $\Fe=0$. A per-quantile shift is a possible extension.
