...
\begin{abstract}
Scientific machine-learning estimators should be evaluated under the information and measurement budget available at inference. We apply this principle to the end-to-end entanglement fidelity of composed quantum channels. Complete Markovian Choi descriptions permit deterministic superoperator composition. At the dimensions studied here, this calculation is numerically exact and faster than the learned models. Measurements of the deployed process also provide a strong reference. Stratified Direct Fidelity Estimation (DFE) is accurate at low shot counts. Our collision benchmark presents a different setting because the input contains reset-bath marginal channels and omits bath retention. A representation ambiguity diameter measures the spread of targets with identical inputs. Half of this diameter lower-bounds the worst-case error for any estimator that uses the same representation. Counterfactual replay covers 2048 collision sequences and 15 indistinguishable retention values. It gives a mean diameter of $0.229$ and a mean pointwise minimax lower bound of $0.115$. A uniform high-memory prior gives a Bayes MAE floor of $0.060$. We then combine a calibrated marginal prior with a same-query DFE pilot. On a fresh split of 4096 samples with independent random streams, a 32-shot neural hybrid reaches MAE $0.0657$. A 64-shot DFE estimate reaches $0.0641$. The 64-shot hybrid reaches $0.0539$, compared with $0.0525$ for 96-shot DFE. The offline costs include finite-shot labels and fusion fitting. They amortise after 192 and 256 deployment queries. The resulting procedure tests input sufficiency and allocates a small measurement budget when the representation omits relevant variables.
\end{abstract}
...
\item We develop a measurement-conditioned prior and DFE fusion rule with an explicit finite-shot calibration contract. It approximately halves the per-query shots needed to reach MAE $\simeq0.065$ and reduces them by one third near MAE $\simeq0.053$.
...
\end{equation}
The triangle inequality applied to two targets near the fibre extrema gives $\Delta_{\phi}(x)\leq |F(z)-h(x)|+|h(x)-F(z')|$. Equation~\eqref{eq:minimax_floor} concerns the pointwise worst case. Average MAE requires a conditional distribution $p(F\mid x)$. Under that distribution, a conditional median minimises MAE and gives the Bayes risk
\begin{equation}
...
The cost comparison contains three types of estimator. A marginal neural prior uses 4096 offline shots and reaches MAE $0.0885$ with zero shots at query time. Stratified DFE has no offline cost and its error decreases approximately as $B^{-1/2}$. The hybrid uses both resources. A 32-shot hybrid and 64-shot DFE reach MAE $0.0657$ and $0.0641$. A 64-shot hybrid and 96-shot DFE reach $0.0539$ and $0.0525$. The hybrid offline costs are 6144 and 8192 shots after counting finite-shot labels and independent fusion pilots. The total costs become equal after $6144/(64-32)=192$ queries for the first comparison and $8192/(96-64)=256$ queries for the second. Training-set simulation is a separate classical cost in this benchmark.
