...
\begin{table}[h]
\centering
\caption{\textbf{Device regime: pooled MAE on $\Fe$.} Exact composition uses 1024 sequences per split. Neural rows use five seeds and labelled-OOD calibration. Stored float32 precision sets the error floor for the full-Choi exact baseline.}
\label{tab:device}
\small
\begin{tabular}{lccc}
\toprule
...
\caption{\textbf{Diagnosing and repairing missing information.} (a) Fixed marginal inputs can correspond to different fidelities as hidden bath retention varies. Curves show representative input groups. (b) The empirical ambiguity distribution yields a mean diameter of $0.229$, a mean pointwise minimax floor of $0.115$, and a uniform-$\eta$ Bayes MAE floor of $0.060$. (c) A same-query DFE pilot supplies information about the deployed process. In the highlighted regimes, the neural hybrid reaches the accuracy of DFE using roughly one half to two thirds of its per-query shots. Error bars show run-to-run standard deviations.}
...
\caption{\textbf{Full amortised measurement cost.} The build cost includes 64 independent 64-shot target labels and 64 independent $B$-shot fusion pilots. The 32-shot hybrid and 64-shot DFE have equal total cost after 192 queries at similar MAE. The corresponding crossing for the 64-shot hybrid and 96-shot DFE occurs after 256 queries.}
...
\begin{table}[h]
\centering
\caption{\textbf{Collision regime: pre-affine distribution metrics for \fidno{}.} Results pool five seeds and 1024 sequences. MAE is the raw point error before affine calibration. ECE uses the predicted median band, and CRPS uses all nine quantiles after the ID-fitted correction.}
...
\caption{\textbf{Bath-retention sweep for the decision rule.} The retention coefficient $\eta$ moves from reset-bath dynamics at $\eta=0$ toward strong memory as $\eta$ approaches one. Each point uses 1024 sequences. Neural curves average five seeds. Analytic and MC-1000 curves use the same split.}
\label{fig:eta_sweep}
\end{figure}
...
Learned surrogates have recently been proposed as inner-loop fidelity proxies for QEC code design~\cite{qec_surrogate}. Our results place these methods in a regime-dependent setting. Analytic estimators can be sufficient for low-infidelity Pauli-like noise. Learned priors become more useful when correlated noise produces corrections absent from marginal estimators. Decoder-level and threshold-level quantities require their own evaluation.
...
All datasets are generated synthetically by scripts in the public repository, \url{https://github.com/shchng9963-droid/fidelityno}. Versioned data and result archives will receive an archival DOI before acceptance. The submission bundle records the code revision and random seeds.

\section*{Code availability}

Code is available on GitHub at \url{https://github.com/shchng9963-droid/fidelityno}.
